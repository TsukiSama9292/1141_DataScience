...
[![Experiments](https://img.shields.io/badge/Experiments-37_Configs-green.svg)](#實驗架構)
...
| **KNN 最佳** | 67.6% | [SVD=200, K=40] | KNN_004 (SVD=200, KNN=40) |
...
#### KNN 鄰居數分析
測試 K=10-100 範圍，**K=40** 為最佳鄰居數：

![KNN K 值分析](reports/figures/knn_k_value_analysis.png)

**關鍵發現**：
- 🎯 K=40 達到最佳平衡（67.6% Hit Rate）
- 📉 K<30 鄰居資訊不足
- 📈 K>50 引入噪音，性能下降
...
採用階段性優化策略，每階段使用前階段最佳結果，共 **37 個配置**：

```
DS (4) → FILTER (6) → SVD (15) → KNN (7) → BIAS (3) → OPT (2)
```
...
│   ├── KNN_001.py ~ KNN_007.py        # KNN階段（7個）
│   ├── BIAS_001.py ~ BIAS_003.py      # 偏差校正階段（3個）
│   ├── OPT_001.py ~ OPT_003.py        # 優化策略階段（2個）
│   ├── VALIDATE_001.py                # 大樣本驗證（20K用戶）
│   └── FULLTEST_001.py                # 全用戶測試（138K用戶）
│
├── log/                               # 實驗結果（JSON格式）
